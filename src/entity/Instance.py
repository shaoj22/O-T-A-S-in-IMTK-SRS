class Instance:
    def __init__(self, order_list=None, tote_list=None, station_list=None, block_list=None, station_buffer_num=5):
        self.order_list = order_list # 订单列表
        self.tote_list = tote_list # 料箱列表
        self.station_list = station_list # 工作站列表
        self.block_list = block_list # 货架列表
    ...
    def gen_tote_and_order_matrix(self):
        ...
        to_matrix = [[0 for i in range(self.tote_num) for o in range(self.order_num)]]
    ...
    def gen_tote_and_block_matrix(self):
        ...
        to_matrix = [[0 for b in range(self.block_num) for i in range(self.tote_num)]]
